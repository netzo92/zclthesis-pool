...
function setup(): PDO {
    ...
    foreach (['zcl_payment_items','zcl_payment_operations','zcl_payment_batches','zcl_payment_control','payouts','earnings','accounts','blocks','coins','zcl_accounting_holds'] as $table) $db->exec("DROP TABLE IF EXISTS $table");
    ...
    $schema = [
        ...
        "CREATE TABLE blocks (id INT PRIMARY KEY,category VARCHAR(32),confirmations INT,coin_id INT DEFAULT 1)$engine",
    ...
    $migration = file_get_contents(__DIR__ . '/../../sql/2026-09-12-zcl-payout-ledger.sql');
    if (!$mysql) {
        $migration = preg_replace('/ALTER TABLE[^;]+;/', '', $migration);
